#ifndef __INFINIOP_ELEMENTWISE_MUSA_H__
#define __INFINIOP_ELEMENTWISE_MUSA_H__

#include "../../../utils.h"
#include "../../devices/musa/common_musa.h"
#include "../../devices/musa/musa_kernel_common.h"
#include "elementwise_musa_api.h"

namespace op::elementwise::musa {
...
__device__ __forceinline__ size_t getOutputIndex(size_t idx, bool is_contiguous, size_t ndim,
    ...
    return is_contiguous ? idx : device::musa::indexToOffset(idx, ndim, shape, strides);
}
    ...
    __device__ __forceinline__ size_t operator()(size_t input_id) const {
        return input_contiguous[input_id]
                 ...
                 : (input_broadcasted[input_id]
                        ? device::musa::indexToReducedOffset(idx, ndim, output_strides, input_strides + input_id * ndim)
                        : device::musa::indexToOffset(idx, ndim, input_shapes + input_id * ndim, input_strides + input_id * ndim));
    }
};
...
/**
 * @brief MUSA kernel for performing elementwise operations on tensors where all inputs share the same data type.
 *
...
INFINIOP_MUSA_KERNEL elementwiseKernel(
    size_t output_size,
    size_t ndim,
...
/**
 * @brief MUSA kernel for performing an elementwise operation on tensors with support
...
struct DeviceImpl::Opaque {
    ...
    Opaque(const std::shared_ptr<device::musa::Handle::Internal> &internal)
        : internal(internal) {}
...
private:
    ...
    infiniStatus_t infoToDevice(
        ...
        CHECK_MUSA(musaMemcpyAsync(workspace, h_inputs_arr, input_arr_size, musaMemcpyHostToDevice, stream));
        CHECK_MUSA(musaMemcpyAsync((void *)d_meta_start, info_meta_start, info.getMetaMemSize(), musaMemcpyHostToDevice, stream));
