...
namespace op::random_sample::musa {
...
utils::Result<size_t> calculateWorkspace(size_t n_) {
    ...
    CHECK_MUSA((radixSort<Tval, Tidx>(
...
template <>
struct MusaTval<fp16_t> {
    using Type = half;
};

// 摩尔平台 bf16 数据类型支持待完善
template <>
struct MusaTval<bf16_t> {
    using Type = half;
};
...
struct Algo {
    ...
    infiniStatus_t argmax(
        ...
        using Tval = typename MusaTval<Tval_>::Type;
