...
__aicore__ inline void RandomSampleKernel<T>::init(GM_ADDR probs, GM_ADDR result, GM_ADDR topk_val_addr, GM_ADDR topk_idx_addr, float random_val, float topp, int topk, float temperature, int32_t n) {
    ...
    // Set GlobalTensor
    _pGM.SetGlobalBuffer(reinterpret_cast<__gm__ T *>(probs), voc_);
    _topk_valGM.SetGlobalBuffer(reinterpret_cast<__gm__ T *>(topk_val_addr), topk_);
    _topk_idxGM.SetGlobalBuffer(reinterpret_cast<__gm__ int64_t *>(topk_idx_addr), topk_);
    _resGM.SetGlobalBuffer(reinterpret_cast<__gm__ int64_t *>(result), 1);

    // Global input and output
    pipe.InitBuffer(_topk_valQue, 1, topkAligned * sizeof(T));
    pipe.InitBuffer(_topk_idxQue, 1, topkIdxAligned * sizeof(int64_t));
    pipe.InitBuffer(_resQue, 1, BYTE_ALIGN); // 32 bytes for aligned
    pipe.InitBuffer(_inBuf, BLOCK_LEN * sizeof(T));
    pipe.InitBuffer(_tmp1Buf, bufferLen * sizeof(T));
    pipe.InitBuffer(_tmp2Buf, bufferLen * sizeof(T));
    pipe.InitBuffer(_tmp3Buf, bufferLen * sizeof(T));
    pipe.InitBuffer(_softmax_OutBuf, topkAligned * sizeof(T));
    pipe.InitBuffer(_inclusive_sum_OutBuf, topkAligned * sizeof(T));
}
...
__aicore__ inline void RandomSampleKernel<T>::RandomSample(LocalTensor<T> &valIn,
    ...
    auto random_val = random_val_ * static_cast<float>(valIn(end - 1));
    for (int i = 0; i < end; i++) {
...
__aicore__ inline void RandomSampleKernel<T>::copyIn() {
    ...
    DataCopy(topkValLocal, _topk_valGM, topkAligned);
    DataCopy(topkIdxLocal, _topk_idxGM, topkIdxAligned);
    ...
    for (int32_t i = 0; i < repeatTimes; i++) {
        ...
        Adds(tmpBuffer, _inBuffer, static_cast<T>(negMax), BLOCK_LEN);
    ...
    if (remainder != 0) {
        ...
        Adds(tmpBuffer, _inBuffer, static_cast<T>(negMax), remainder);
