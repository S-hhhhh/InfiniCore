...
private:
    ...
    __device__ __forceinline__ cuda_bfloat16 gelu_backward_bfloat16(const cuda_bfloat16 &input, const cuda_bfloat16 &grad_output) const {
        const cuda_bfloat16 sqrt_2_over_pi = __float2bfloat16(0.7978845608028654f);
        const cuda_bfloat16 coeff = __float2bfloat16(0.044715f);
        const cuda_bfloat16 half_val = __float2bfloat16(0.5f);
        const cuda_bfloat16 one = __float2bfloat16(1.0f);
        const cuda_bfloat16 three = __float2bfloat16(3.0f);
        
        cuda_bfloat16 x = input;
        cuda_bfloat16 x_squared = __hmul(x, x);
        cuda_bfloat16 x_cubed = __hmul(x_squared, x);
        cuda_bfloat16 tanh_arg = __hmul(sqrt_2_over_pi, __hadd(x, __hmul(coeff, x_cubed)));
        cuda_bfloat16 tanh_val = htanh(tanh_arg);
        
        cuda_bfloat16 sech_squared = __hsub(one, __hmul(tanh_val, tanh_val));
        cuda_bfloat16 arg_derivative = __hmul(sqrt_2_over_pi, __hadd(one, __hmul(__hmul(three, coeff), x_squared)));
        cuda_bfloat16 gelu_derivative = __hadd(__hmul(half_val, __hadd(one, tanh_val)), 
                                             __hmul(__hmul(x, half_val), __hmul(sech_squared, arg_derivative)));
