...
private:
    ...
    __device__ __forceinline__ T sigmoid(const T &x) const {
        ...
        } else if constexpr (std::is_same_v<T, __hpcc_bfloat162>) {
        ...
        } else if constexpr (std::is_same_v<T, __hpcc_bfloat16>) {
...
public:
    ...
    __device__ __forceinline__ T operator()(const T &up, const T &gate) const {
        if constexpr (std::is_same_v<T, half2>) {
            ...
            __hpcc_bfloat162 sig = sigmoid(gate);
            ...
            __hpcc_bfloat16 sig = sigmoid(gate);
