...
infiniStatus_t launchKernel(
    ...
    // 计算对齐后的线程块大小（Block Size）以适配MUSA架构的Warp特性
    ...
    // 1. 是32的整数倍（适配 MUSA 的 Warp 调度机制）
    // 2. 不小于实际需要处理的元素数（block_len_total）
    // 3. 向上取整，数学等效：ceil(n / 32) * 32
...
infiniStatus_t Descriptor::calculate(
    ...
    // 准备参数
    auto params_result = prepareRearrangeParams(_meta, std::min(MUSA_BLOCK_SIZE_1024, max_threads));
    ...
    if (block_size <= MUSA_BLOCK_SIZE_512) {
        status = launchKernel<MUSA_BLOCK_SIZE_512>(y, x, grid_size, params, _meta.unit(), musa_stream);
    } else if (block_size <= MUSA_BLOCK_SIZE_1024) {
        status = launchKernel<MUSA_BLOCK_SIZE_1024>(y, x, grid_size, params, _meta.unit(), musa_stream);
    } else {
