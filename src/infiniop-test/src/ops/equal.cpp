...
struct Test::Attributes {
    ...
    std::shared_ptr<Tensor> c;   // bool output
    std::shared_ptr<Tensor> ans; // bool reference
};
...
std::shared_ptr<infiniop_test::Result> Test::run(
    ...
    double elapsed_time = benchmark(
        [=]() {
            infiniopEqual(op_desc, workspace, workspace_size,
                          c->data(),
                          a->data(),
                          b->data(),
                          nullptr);
...
std::vector<std::string> Test::tensor_names() { return {"a", "b", "c", "ans"}; }

std::vector<std::string> Test::output_names() { return {"c"}; }
...
std::string Test::toString() const {
    ...
    oss << "- c (bool): " << _attributes->c->info() << std::endl;
